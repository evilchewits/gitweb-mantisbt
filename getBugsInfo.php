...
function getBugsInfoJSONPResponse( $bugsString ) {
	...
	$query = "SELECT id, status, summary FROM `".$t_bug_table."` WHERE id IN (" . $bugs_list . ") ORDER BY FIELD(id, " . $bugs_list . ")";
	...
	$trackerHref = config_get( 'path' ) . "view.php?id=";

	header("Content-Type: application/javascript; charset=utf-8");

	echo 'bugtrackerConnection_callback( { "statuses": { ' . substr($statuses, 0, -2) . ' }, "bugsInfo" : { ' . substr($json, 0, -2) . ' } } );';
}
